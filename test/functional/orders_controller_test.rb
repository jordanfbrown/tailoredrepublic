...
class OrdersControllerTest < ActionController::TestCase
  ...
  test "A user who has created their account less than an hour ago should be able to update their name, email, and
        password from the new order form" do
    set_cart_cookie(:two_suits)
    user = users(:user_without_stripe)
    sign_in :user, user
    post :review, {
    ...
    user.reload
    assert assigns(:can_edit_account_info)
    assert_equal 'Test Test', user.name
    assert_equal 'test1001@test.com', user.email
  end

  test "When the total cost of an order is $0, an order ID is generated by us instead of Stripe, and the order still
       works correctly" do
    set_cart_cookie(:one_suit)
    user = users(:user_with_stripe)
    sign_in :user, user
    params = @order_params
    post :create, {
  ...
  test "A user with a referral credit should not get a discount if their order only contains accessories" do
    set_cart_cookie(:accessory_cart)
    user = users(:user_with_referral_credit)
    referral_credit = user.referral_credit
    sign_in :user, user
    post :create, {
      order: @order_params,
      card_radio: 'use_saved_card'
    ...
    user.reload
    assert_equal referral_credit, user.referral_credit
    assert_nil assigns(:order).referral_discount
  end
